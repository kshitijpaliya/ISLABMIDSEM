...
class KeyManagementClient:
    ...
    def send_request(self, request):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            # Ensure the request is a valid JSON string before sending
    ...
    def run(self):
        ...
        while True:
            choice = self.menu()
            if choice == '1':
                print("Sending generate key request...")
                response = self.send_request({"action": "generate_key", "username": username})
